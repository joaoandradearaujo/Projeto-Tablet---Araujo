...
class EquipmentInspectionApp(tk.Tk):
    def __init__(self):
        ...
        self.clientes_options = self.load_clientes_data()
        ...
        self.componente_options = self.load_componente_data()
    ...
    def load_clientes_data(self):
        ...
        try:
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):  # Assumindo que a primeira linha é o cabeçalho
                clientes.append(row[1])  # Assume que o nome do cliente está na segunda coluna
    ...
    def load_componente_data(self):
        ...
        try:
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):  # Assumindo que a primeira linha é o cabeçalho
                componente.append(row[3])  # Assume que o nome do componente está na quarta coluna
        except Exception as e:
    ...
    def create_trabalho_widgets(self):
        ...
        for idx, field in enumerate(fields_trabalho):
            ...
            if field == "CLIENTE":
                ...
                var = tk.StringVar()
                entry = ttk.Combobox(self.tab_trabalho, width=40, textvariable=var, values=self.clientes_options, state='readonly')
                entry.current(0)
                ...
                var = tk.StringVar()
                entry = ttk.Combobox(self.tab_trabalho, width=40, textvariable=var, values=self.componente_options, state='readonly')
                entry.current(0)
                entry.grid(row=idx, column=1, padx=10, pady=5, sticky=tk.W)
                self.fields_entries_trabalho[field] = var
